...
$profesor = new Profesor($prof['dni'], $prof['apellidos'], $prof['nombre'], $prof['email'], $prof['password']
        , $prof['tutor_Grupo'], $prof['rol']);
if (isset($_POST['enviar'])) {
    // Si el usuario pulsa el boton de enviar
    $_SESSION["editado"] = 1;
    $validar_profesor = new ValidarRegistroProfesor($_POST['apellidos']
            , $_POST['nombre'], $_POST['email'], $_POST['password1'], $_POST['password2']);
    if ($validar_profesor->registroCorrecto()) {
        ...
        if ($prof['passoword'] == $_POST['password1']) {
            $profesor = new Profesor($prof['dni'], $_POST['apellidos']

...
<body>
    ...
    <div class="fondo">
        <div class="matricula mt-4">
            <h1 style="text-align: right; color: green; font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif;">1BTOCIENCIAS</h1>
            <form id="matriculas" action="../../../generate_pdf.php" method="post">
                <input type="hidden" name="curso" value="1BTOCIENCIAS">
                ...
                <input type="hidden" name="preferenciaClasesObligatoria" id="preferenciaClasesObligatoria" value="Matemáticas I">
  ...
  <div class="form-group mb-2">
                ...
                <table class="table table-bordered table-striped table-hover text-center">
                    ...
                    <tbody>
                        <?php  
                            $select = "SELECT cod_asignatura, nombre, horas, curso, tipo FROM Asignaturas WHERE curso LIKE '1BTOCIENCIAS' AND tipo LIKE '%obligatoria'";
                            $resul = $db->query($select);                            
                            // Utilizamos un bucle while para recorrer todas las filas que devuelve la consulta
                            while ($asignatura = $resul->fetch(PDO::FETCH_ASSOC)) {
                                echo '<tr>';
                                // Recorremos las columnas de la fila actual
                                    echo '<td id="asignaturas_obligatorias'.$asignatura['cod_asignatura'].'">'.$asignatura['nombre'].'</td>';
                ...
                <table class="table table-bordered table-striped table-hover text-center">
                    ...
                    <tbody>
                        <?php  
                          $select = "SELECT cod_asignatura, nombre, horas, curso, tipo FROM Asignaturas WHERE curso LIKE '1BTOCIENCIAS' AND tipo LIKE '%modalidad'";
                          $resul = $db->query($select);
                          $idClasesModalidad = 1;
                          // Utilizamos un bucle while para recorrer todas las filas que devuelve la consulta
                          while ($asignatura = $resul->fetch(PDO::FETCH_ASSOC)) {
                ...
                <table class="table table-bordered table-striped table-hover text-center">
                    ...
                    <?php  
                            $select = "SELECT cod_asignatura, nombre, horas, curso, tipo FROM Asignaturas WHERE curso LIKE '1BTO' AND tipo LIKE '%optativas'";
                            $resul = $db->query($select);
                            $idClasesOptativas = 1;
                            // Utilizamos un bucle while para recorrer todas las filas que devuelve la consulta
                            while ($asignatura = $resul->fetch(PDO::FETCH_ASSOC)) {

...
<body>
    ...
    <div class="fondo">
        <div class="matricula mt-4">
            <h1 style="text-align: right; color: green; font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif;">1BTO-Ciencias</h1>
            <form id="matriculas" action="../../../generate_pdf.php" method="post">
                <input type="hidden" name="curso" value="1BTO-Ciencias">
                ...
                <table class="table table-bordered table-striped table-hover text-center">
                    ...
                    <tbody>
                        <?php  
                            $select = "SELECT cod_asignatura, nombre, horas, curso, tipo FROM asignaturas WHERE curso LIKE 'BTOCIENCIAS1' AND tipo LIKE '%obligatoria'";
                            $resul = $db->query($select);                            
                            // Utilizamos un bucle while para recorrer todas las filas que devuelve la consulta
                            while ($asignatura = $resul->fetch(PDO::FETCH_ASSOC)) {
                                echo '<tr>';
                ...
                <table class="table table-bordered table-striped table-hover text-center">
                    ...
                    <tbody>
                        <?php  
                            $select = "SELECT cod_asignatura, nombre, horas, curso, tipo FROM asignaturas WHERE curso LIKE 'BTOCIENCIAS1' AND tipo LIKE '%modalidad'";
                            $resul = $db->query($select);
                            $idClasesModalidad = 1;
                            // Utilizamos un bucle while para recorrer todas las filas que devuelve la consulta
                            while ($asignatura = $resul->fetch(PDO::FETCH_ASSOC)) {

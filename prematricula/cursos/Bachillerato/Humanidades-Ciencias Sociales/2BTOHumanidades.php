...
<body>
    ...
    <div class="fondo">
        <div class="matricula mt-4">
            <h1 style="text-align: right; color: green; font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif;">2BTOHUMCSO</h1>
            <form id="matriculas" action="../../../generate_pdf.php" method="post">
                <input type="hidden" name="curso" value="2BTOHUMCSO">
                ...
                <table class="table table-bordered table-striped table-hover text-center">
                    ...
                    <?php  
                            $select = "SELECT cod_asignatura, nombre, horas, curso, tipo FROM Asignaturas WHERE curso LIKE '2BTOHUMCSO' AND tipo LIKE '%obligatoria'";
                            $resul = $db->query($select);                            
                            // Utilizamos un bucle while para recorrer todas las filas que devuelve la consulta
                            while ($asignatura = $resul->fetch(PDO::FETCH_ASSOC)) {
                                echo '<tr>';
                ...
                <table class="table table-bordered table-striped table-hover text-center">
                    ...
                    <?php  
                          $select = "SELECT cod_asignatura, nombre, horas, curso, tipo FROM Asignaturas WHERE curso LIKE '2BTOHUMCSO' AND tipo LIKE '%modalidad'";
                          $resul = $db->query($select);
                          $idClasesModalidad = 1;
                          // Utilizamos un bucle while para recorrer todas las filas que devuelve la consulta
                          while ($asignatura = $resul->fetch(PDO::FETCH_ASSOC)) {

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    //Datos de los Progenitores o Tutores Legales
        ...
        $pdf->SetFont('Arial', '', 10);
        $pdf->MultiCell(100, 5, mb_convert_encoding('Nombre y Apellidos: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($nombreApellidosProgenitor1, "ISO-8859-1", "UTF-8"), 0, 1);
        $pdf->Cell(0, 5, mb_convert_encoding('DNI: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($dniProgenitor1, "ISO-8859-1", "UTF-8"), 0, 1);
        $pdf->Cell(0, 5, mb_convert_encoding('Teléfono: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($telefonoProgenitor1, "ISO-8859-1", "UTF-8"), 0, 1);
        $pdf->MultiCell(100, 5, mb_convert_encoding('Correo electrónico: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($correoProgenitor1, "ISO-8859-1", "UTF-8"), 0, 1);
        if(!empty($dniProgenitor2)){
        $pdf->SetXY(110, 85); // Ajustar x e y según sea necesario

        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(0, 5, mb_convert_encoding('Progenitor 2 o Tutor/a 2', "ISO-8859-1", "UTF-8"), 0, 1);
        $pdf->SetFont('Arial', '', 10);
        $pdf->SetXY(110, $pdf->GetY()); // Ajustar x e y según sea necesario
        $pdf->MultiCell(0, 5, mb_convert_encoding('Nombre y Apellidos: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($nombreApellidosProgenitor2, "ISO-8859-1", "UTF-8"), 0, 1);
        $pdf->SetXY(110, $pdf->GetY()); // Ajustar x e y según sea necesario
        $pdf->Cell(0, 5, mb_convert_encoding('DNI: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($dniProgenitor2, "ISO-8859-1", "UTF-8"), 0, 1);
        $pdf->SetXY(110, $pdf->GetY()); // Ajustar x e y según sea necesario
        $pdf->Cell(0, 5, mb_convert_encoding('Teléfono: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($telefonoProgenitor2, "ISO-8859-1", "UTF-8"), 0, 1);
        $pdf->SetXY(110, $pdf->GetY()); // Ajustar x e y según sea necesario
        $pdf->MultiCell(0, 5, mb_convert_encoding('Correo electrónico: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($correoProgenitor2, "ISO-8859-1", "UTF-8"), 0, 1);
        }
    ...
    $pdf->SetFont('Arial', '', 10);
    $direccion = mb_convert_encoding('C/Avda. Plaza: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($calle, "ISO-8859-1", "UTF-8") . mb_convert_encoding(', Número: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($numero, "ISO-8859-1", "UTF-8");

    if (!empty($portal)) {
        $direccion .= mb_convert_encoding(', Portal: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($portal, "ISO-8859-1", "UTF-8");
    }
    if (!empty($piso)) {
        $direccion .= mb_convert_encoding(', Piso: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($piso, "ISO-8859-1", "UTF-8");
    }
    if (!empty($puerta)) {
        $direccion .= mb_convert_encoding(', Puerta: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($puerta, "ISO-8859-1", "UTF-8");
    }
    
    $direccion .= mb_convert_encoding(', Código Postal: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($codigoPostal, "ISO-8859-1", "UTF-8");
    $direccion .= mb_convert_encoding(', Municipio: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($municipio, "ISO-8859-1", "UTF-8");
    $direccion .= mb_convert_encoding(' ,Provincia: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($provincia, "ISO-8859-1", "UTF-8");

  
    $pdf->MultiCell(0, 5, $direccion, 0, 1);
    ...
    if (!empty($centro) && !empty($localidadCentro) && !empty($provinciaCentro) && !empty($curso_antiguo)){
        ...
        $pdf->SetFont('Arial', '', 10);
        $pdf->MultiCell(0, 5, mb_convert_encoding('El solicitante, durante el curso '.($year_current-1)." / ".$year_current.', estuvo matriculado en el centro: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($centro, "ISO-8859-1", "UTF-8").  mb_convert_encoding(' de la localidad: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($localidadCentro, "ISO-8859-1", "UTF-8"). mb_convert_encoding(' ,provincia: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($provinciaCentro, "ISO-8859-1", "UTF-8") . mb_convert_encoding(' en el curso: ', "ISO-8859-1", "UTF-8") . mb_convert_encoding($curso_antiguo, "ISO-8859-1", "UTF-8"), 0, 1);  
    ...
    $pdf->MultiCell(0, 5, mb_convert_encoding('Resguardo de matrícula que ha presentado el alumno/a '.$nombre.' '.$primerApellido.' '.$segundoApellido.' para 1º ESO para el curso académico '.$year_current." / ".$year_next.' en el IES "Julio Verne" el día '.date('d-m-Y'), "ISO-8859-1", "UTF-8"));
    $pdf->SetFont('Arial', 'B', 10);
    ...
    $pdf->MultiCell(0, 5, mb_convert_encoding('* Si el alumno/a ha estado matriculado en el curso '.($year_current-1)." / ".$year_current.' en un centro fuera de Castilla-La Mancha, deberá aportar en el momento de matricularse un certificado oficial académico de dicho curso, expedido por el centro.', "ISO-8859-1", "UTF-8"));
    $pdf->SetFont('Arial', '', 10);
    $pdf->ln(); 
    ...
    $pdf->Cell(95, 15, mb_convert_encoding('Legitimación', "ISO-8859-1", "UTF-8"), 1, 0, 'C', true);
